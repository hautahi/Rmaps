...
This is a brief demonstration of common mapping techniques using spatial analysis tools in R. The goal here is to go from knowing nothing about shapefiles, to being able to create meaningful and good looking maps. For this exercise, I use the crime data from Portland, Oregon that was provided by the National Institute of Justice for their [crime prediction](https://nij.gov/funding/Pages/fy16-crime-forecasting-challenge.aspx) competition. You can download the data and R scripts for this exercise from its [Github repository](https://github.com/hautahi/RGIS_Tutorial).

### Loading Spatial Objects from Shape Files

Spatial data usually comes in the form  of shapefiles. We can load shapefiles using the `readOGR` function from the `rgdal` package.^[The `maptools` package is also popular for reading shapefiles.]
...
If, like me, you're used to regular R dataframes, you'll find that spatial objects (which are represented by [S4 objects](http://adv-r.had.co.nz/S4.html) under the hood) are a little weird. But their weirdness allows a lot of flexibility. The first thing to know is that each spatial object belongs to one of many different "classes". The most elementary type of spatial data are *Points*, which denote single point locations. *Lines* are a set of points connected by straight line segments that can be used to denote things like rivers or state boundaries. *Polygons* are shapes that denote area, and a *Grid* is a collection of cells organized into a regular grid. Running the `class()` command shows that the `port` object is a Spatial Polygons Data Frame and the `crime` object is a Spatial Points Data Frame. Let's plot each of these objects to see the difference.
...
It turns out that some of the crimes contained in `crime` occur outside the city of portland, as described by the `port` polygons. We can `clip` the crimes that occur outside the city using simple base R functions. Plotting the clipped and non-clipped data illustrates the point.
...
When working with spatial data, we often want to perform operations on data that we can use to make "cross polygon" comparisons. The `aggregate` function is very useful for this because it allows us to perform operations "by" polygons. The following code counts the number of crimes contained in `crime` by the polygons defined in `port`.
...
The `summary(crime_agg)` command shows that new spatial object `crime_agg` is a polygon dataframe which has the same projection properties and "bounding box" as the `port` object. This is because the aggregate function simply counted the number of crimes whose x,y coordinates were in each of the polygons described by the `port` object. `crime_agg` only has one data attribute, however, which corresponds to the crime counts within each police district. Because there are only 60 police districts, we can actually just print the `CATEGORY` data column, which lists the crime count within each of the 60 districts. Each entry represents the number of crimes committed in that row's corresponding polygon.
...
There are a number of very useful packages for plotting spatial data, including [ggmap](https://cran.r-project.org/web/packages/ggmap/index.html), and [tmap](https://cran.r-project.org/web/packages/tmap/index.html). Here, we will use Leaflet. It isn't the simplest package, but it offers a lot of power, is apparently the [leading open-source](http://leafletjs.com/) mapping library and, in my opinion, looks cool. The first thing to do is to transform the projection of the coordinates.

```{r}
crime_agg84 <- spTransform(crime_agg, CRS("+init=epsg:4326")) # Reproject coordinates
```
...
```{r}
library(leaflet)
qpal <- colorBin("Reds", crime_agg84$CATEGORY, bins=5)
```

The syntax of the Leaflet plot is actually very similar to the popular ggplot syntax in the sense that you add layers to the map in steps. The first is to create a leaflet object from the `crime_agg84` spatial object. We then use the `addPolygons` layer to overlay the police district outlines onto the map. We use the pallette mapping created above in the `fillColor` option. Finally, we add a a legend.

```{r plot1, warning=FALSE, message=FALSE,fig.align='center', results='hide'}
leaflet(crime_agg84) %>%
  addPolygons(stroke = TRUE,opacity = 1,fillOpacity = 0.5, smoothFactor = 0.5,color="black",fillColor = ~qpal(CATEGORY),weight = 1) %>%
...
When working with spatial data, we would often like to work within our own evenly-spaced grid. The `raster` package is a simple way to make grids.
...
The first step in the code above is to define an `extent` object, which basically defines the "boundaries" of our raster grid. Here, I've used the boundaries of the `port` object, which is accessed by the `bbox` command. The next line creates the raster object with our given extent. We then specify how many cells we'd like the raster to have. Here we specify the dimensions of the raster, which splits the extent into a 40 x 40 square grid. Finally, I give the raster cell the same projection orientation as `port`. The resulting `r` is a raster object, which we then convert into a spatial polygon object `g`.

Inspecting `g`, we see that it has the same min and max x,y coordinates as port, as a result of the extent we defined above. There are no data attributes because we haven't defined any.
...
crime_agg841 <- spTransform(crime_agg1, CRS("+init=epsg:4326")) # reproject
qpal <- colorBin("Reds", crime_agg841$CATEGORY, bins=5)
leaflet(crime_agg841) %>%
  addPolygons(stroke = TRUE,opacity = 1,fillOpacity = 0.5, smoothFactor = 0.5,color="black",fillColor = ~qpal(CATEGORY),weight = 0.5) %>%
  addLegend(values=~CATEGORY,pal=qpal,title="Crime Count")
```
...
library(rgeos)
portland = gUnaryUnion(port, port$dummy) # Melt Portland districts into one polygon
map <- gIntersection(p,portland,byid = TRUE,drop_lower_td = TRUE) # Take the intersection of the grid and the Portland map
```
...
crime_agg842 <- spTransform(crime_agg2, CRS("+init=epsg:4326")) # reproject
qpal <- colorBin("Reds", crime_agg842$CATEGORY, bins=5)
plot3 <-
  leaflet(crime_agg842) %>%
  addPolygons(stroke = TRUE,opacity = 1,fillOpacity = 0.5, smoothFactor = 0.5,color="black",fillColor = ~qpal(CATEGORY),weight = 0.5) %>%
  addLegend(values=~CATEGORY,pal=qpal,title="Crime Count")
```
...
```{r results='hide'}
cent <- gCentroid(crime_agg842) # Find center of map
...
```{r}
writeOGR(crime_agg842, dsn='output_data', layer='Portland', driver="ESRI Shapefile", overwrite_layer=T)
```
...
- An [intro to spatial analysis in R](https://cran.r-project.org/doc/contrib/intro-spatial-rl.pdf) by Robin Lovelace and others.

- A [spatial data tutorial](https://pakillo.github.io/R-GIS-tutorial/) by Francisco Rodriguez-Sanchez.

- A [post](http://www.nickeubank.com/gis-in-r/) by Nick Eubank.